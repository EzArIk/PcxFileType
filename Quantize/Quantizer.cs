            ...
            try
            {
                ...
                for (int row = 0; row < height; row++)
                {
                    ...
                    for (int col = 0; col < width; ++col)
                    {
                        ...
                        target.B = Int32Util.ClampToByte(srcPixel.B - ((errorThisRowB[col] * weight) / 8));
                        target.G = Int32Util.ClampToByte(srcPixel.G - ((errorThisRowG[col] * weight) / 8));
                        target.R = Int32Util.ClampToByte(srcPixel.R - ((errorThisRowR[col] * weight) / 8));
                        target.A = srcPixel.A;
